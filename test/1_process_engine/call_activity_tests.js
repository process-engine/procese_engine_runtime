...
describe('Call activity tests', () => {
  ...
  it('should execute a process which uses a call activity to increment a given token', async () => {
    ...
    should(result.tokenPayload).have.property('history');
    should(result.tokenPayload).have.property('current');

    const task1Result = result.tokenPayload.history.Task1;
    const finalResult = result.tokenPayload.current;
  ...
  it('should call an erroneous call activity, whose error will be handled by the call activity itself', async () => {
    ...
    should(result).have.property('tokenPayload');
    should(result.tokenPayload).have.property('current');
    should(result.tokenPayload.current).be.match(/error caught by subprocess/i);
  });
  ...
  it('should call an erroneous call actvity, whose error will be handled by a boundary event in the calling process', async () => {
    ...
    should(result).have.property('tokenPayload');
    should(result.tokenPayload).have.property('current');
    should(result.tokenPayload.current).be.match(/error caught by main process/i);
  });
});
